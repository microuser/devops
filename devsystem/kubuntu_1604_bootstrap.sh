...
then
    dialog --infobox "Enabling vagrant-bindfs for nfs shares"
    vagrant plugin install vagrant-bindfs
fi  

if ! vagrant plugin list | grep 'vagrant-hostsupdater' >/dev/null
then
    dialog --infobox "Enabling vagrant-hostsupdater"
    vagrant plugin install vagrant-hostsupdater
fi  

if ! vagrant plugin list | grep 'vagrant-triggers' >/dev/null
then
    dialog --infobox "Enabling vagrant-triggers"
...
then	
  ...
  noOpMode || sudo add-apt-repository ppa:webupd8team/java
